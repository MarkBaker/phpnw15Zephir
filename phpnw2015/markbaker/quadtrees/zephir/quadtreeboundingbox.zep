...
class QuadTreeBoundingBox
{
     ...
     */
    public function __construct(<QuadTreeXYPoint> centrePoint, float width = 1.0, float height = 1.0) -> void
    {
        if !is_numeric(width) || !is_numeric(height) {
            throw new \InvalidArgumentException("Bounding box width and height must be numeric values");
        }

        let this->centrePoint = centrePoint;
     ...
     */
    protected function startX() -> float
    {
     ...
     */
    protected function startY() -> float
    {
     ...
     */
    protected function isXinRange(float pointX) -> boolean
    {
     ...
     */
    protected function isYinRange(float pointY) -> boolean
    {
     ...
     *                                         this bounding box?
     */
    public function containsPoint(<QuadTreeXYPoint> point) -> boolean
    {
     ...
     */
    public function intersects(<QuadTreeBoundingBox> boundary) -> boolean
    {
        return this->isXinRange(boundary->startX()) || this->isXinRange(boundary->endX()) ||
     ...
     *                                                this bounding box?
     */
    public function encompasses(<QuadTreeBoundingBox> boundary) -> boolean
    {
        return boundary->startX() <= this->startX() && boundary->endX() >= this->endX() &&
